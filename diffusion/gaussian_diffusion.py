    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None, dataset=None):
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:     # LossType.MSE
            ...
            if self.lambda_vel > 0.:        # 0.0
                ...
                terms["vel_mse"] = self.masked_l2(target_vel[:, :, :, :],
                                                  model_output_vel[:, :, :, :],
                                                  mask[:, :, :, 1:])  # mean_flat((target_vel - model_output_vel) ** 2)
            if self.lambda_acc > 0.:        # 0.0
                target_vel = (target[..., 1:] - target[..., :-1])
                target_acc = (target_vel[..., 1:] - target_vel[..., :-1])
                model_output_vel = (model_output[..., 1:] - model_output[..., :-1])
                model_output_acc = (model_output_vel[..., 1:] - model_output_vel[..., :-1])
                terms["acc_mse"] = self.masked_l2(target_acc[:, :, :, :], #
                                                  model_output_acc[:, :, :, :],
                                                  mask[:, :, :, 1:-1])  # mean_flat((target_vel - model_output_vel) ** 2)
            if self.lambda_angle > 0.:        # 0.0
                target_1 = target.permute(0,2,3,1)
                model_output_1 = model_output.permute(0,2,3,1)

                bs, nfeats, nframes, njoints = target_1.shape

                target_a = target_1.reshape(bs, nfeats, nframes, -1, 9)
                target_a = target_a.reshape(bs, nfeats, nframes, -1, 3, 3)
                model_output_a = model_output_1.reshape(bs, nfeats, nframes, -1, 9)
                model_output_a = model_output_a.reshape(bs, nfeats, nframes, -1, 3, 3)

                target_a_1, model_output_a_1 = (matrix_to_euler_angles(target_a), matrix_to_euler_angles(model_output_a))
                target_a_1 = target_a_1.reshape(bs, nfeats, nframes, -1)
                model_output_a_1 = model_output_a_1.reshape(bs, nfeats, nframes, -1)
                # import ipdb
                # ipdb.set_trace()
                target_a_1 = target_a_1.permute(0,3,1,2)
                model_output_a_1 = model_output_a_1.permute(0,3,1,2)

                terms["angle_mse"] = self.masked_l2(target_a_1[:, :, :, :], #
                                                  model_output_a_1[:, :, :, :],
                                                  mask[:, :, :, :])
            ...
            terms["loss"] = terms["rot_mse"] + terms.get('vb', 0.) +\
                            ...
                            (self.lambda_angle * terms.get('angle_mse', 0.)) +\
