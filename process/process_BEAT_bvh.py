...
def pose2bvh_bugfix(save_path, filename_prefix, poses, pipeline='./resource/data_pipe_30fps.sav', gt=False):
    writer = BVHWriter()
    ...
    if not gt:
        ...
        if not os.path.exists(out_bvh_path):
            print('generate bvh')
            with open(out_bvh_path, 'w') as f:
                writer.write(bvh_data[0], f)
    return out_euler

def pose2euler(poses, gt=False, fixroot=False, normalize_root=False, upper_body=False): 
    ...
    if not gt and fixroot:
        zeros_euler = np.zeros((out_poses.shape[0], 3))
        out_euler[:, :3] = zeros_euler
    
    if not gt and normalize_root:
        pos_mean = np.mean(out_euler[:, :3], axis=0)
        pos_std = np.std(out_euler[:, :3], axis=0)
        # zeros_euler = np.zeros((out_poses.shape[0], 3))
        out_euler[:, :3] = (out_euler[:, :3] - pos_mean) / pos_std
    
    if not gt and upper_body:
        # zeros_euler = np.zeros((out_poses.shape[0], 36))
        root_euler = out_euler[0, 0:6]
        first_euler = out_euler[0, 192:]
        first_euler = np.expand_dims(first_euler,0).repeat(out_euler.shape[0],axis=0)
        root_euler = np.expand_dims(root_euler,0).repeat(out_euler.shape[0],axis=0)
        out_euler[:, 0:6] = root_euler
        out_euler[:, 192:] = first_euler

    return out_euler

def pose2bvh(save_path, filename_prefix, poses, pipeline='./resource/data_pipe_30fps.sav', gt=False):
    writer = BVHWriter()
    pipeline = jl.load(pipeline)
    ...
    if not gt:
        out_bvh_path = os.path.join(save_path, filename_prefix + '_generated.bvh')
    if gt:
        out_bvh_path = os.path.join(save_path, filename_prefix + '_gt.bvh')
    with open(out_bvh_path, 'w') as f:
        writer.write(bvh_data[0], f)

def pose2bvh_style(save_path, filename_prefix, poses, pipeline='./resource/data_pipe_30fps.sav', gt=False, suffix=''):
    writer = BVHWriter()
    pipeline = jl.load(pipeline)
    ...
    if not gt:
        out_bvh_path = os.path.join(save_path, filename_prefix + '_' + suffix +'.bvh')
    if gt:
        out_bvh_path = os.path.join(save_path, filename_prefix + '_gt.bvh')
    with open(out_bvh_path, 'w') as f:
        writer.write(bvh_data[0], f)
...
def load_audio(audiofile, wavlm_model, cfg, device=torch.device('cuda:0')):
    ...
    prosody = extract_prosodic_features(audiofile, format="wav")  # (7199, 4)
...
def make_gesture_dataset(base_path, save_path, split, preload=False, wavlm_model=None, cfg=None, word2vector=None,
        ...
        for speaker in os.listdir(base_path):
            ...
            for v_i, bvh_file in enumerate(bvh_files):
                ...
                if os.path.exists(os.path.join(emo_save_path, name + ".npy")):
                    ...
                    if not flag:
                        if 'v0' in version:
                            poses, euler = process_bvh_bugfix(bvh_file)
                    ...
                    try:    
                        ...
                        print(f"# ---- file not found for Emo {name}, skip all files with the same id ---- #")
                        continue
                ...
                if os.path.exists(os.path.join(sem_save_path, name + ".npy")):
                    ...
                    try:
                        ...
                        print(f"# ---- file not found for Sem {name}, skip all files with the same id ---- #")
                        continue
    ...
    if preload:
        with h5py.File(f"BEAT_" + version + "_" + split + ".h5", "w") as h5:
            total_index = 0
            gesture_path = motion_save_path

            print('Processing  {}'.format(gesture_path))
            bvh_files = sorted(glob.glob(gesture_path + "/*.npy"))

            for v_i, bvh_file in enumerate(bvh_files):
                print(bvh_file)
                name = os.path.split(bvh_file)[1][:-4]
                speaker = name.split('_')[0]
                print(f"Processing {v_i + 1}/{len(bvh_files)}: {name}   Speaker {speaker}")
                ...
                clip_len = min(poses.shape[0], wav.shape[0], tsv.shape[0], emo.shape[0])
                poses = poses[:clip_len]
                wav = wav[:clip_len]
                tsv = tsv[:clip_len]
                emo = emo[:clip_len]
                g_data.create_dataset("speaker_id", data=[speaker])
...
def generate_data(base_path, save_path, split, preload=False, wavlm_model=None, cfg=None, word2vector=None,
                         device=torch.device('cuda:0'), version='v0'):
    audio_save_path = os.path.join(save_path, 'audio_BEAT')
    text_save_path = os.path.join(save_path, 'text_BEAT')
    ...
    wav_files = sorted(glob.glob(base_path + "/*.wav"))
...
def main():
    ...
    if args.step == "step1":
        ...
        make_gesture_dataset(args.db_path, args.save_path, split, preload, wavlm_model, cfg, word2vector, device, args.version)
